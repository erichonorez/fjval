...
public final class DefaultErrors {
    ...
    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> gt(T b) { 
        return Validators.gt(b, v -> error(COMPARABLE_NOT_GREATER_THAN, b, v));
    }

    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> eq(T b) {
        return Validators.eq(b, v -> error(COMPARABLE_NOT_EQUAL_TO, b, v));
      }

    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> gte(T b) {
        return Validators.gt(b, v -> error(COMPARABLE_NOT_GREATER_THAN_OR_EQUAL_TO, b, v));
    }

    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> lt(T b) {
        return Validators.gt(b, v -> error(COMPARABLE_NOT_LOWER_THAN, b, v));
     }

    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> lte(T b) { 
        return Validators.gt(b, v -> error(COMPARABLE_NOT_LOWER_THAN_OR_EQUAL_TO, b, v));
    }

    public static <T extends Comparable<T>> KeyedValidator<T, ValidationError> between(T a, T b) {
        return Validators.gt(b, v -> error(COMPARABLE_NOT_BETWEEN_BOUNDS, a, b, v));
     }

    public static <E> KeyedValidator<String, ValidationError> matches(String regex) {
        return Validators.matches(regex, v -> error(STRING_DONT_MATCH, regex, v));
     }

    public static <E> KeyedValidator<String, ValidationError> contains(CharSequence b) {
        return Validators.contains(b, v -> error(STRING_DONT_CONTAIN, b, v));
     }

    public static <T, E> KeyedValidator<T, ValidationError> in(Set<T> xs) {
        return Validators.in(xs, v -> error(VAULE_NOT_IN_SET, xs, v));
     }

    public static <T, E> KeyedValidator<T, ValidationError> equalsTo(T b) { 
        return Validators.equals(b, v -> error(VALUE_NOT_EQUAL_TO, b, v));
    }

    public static <E> KeyedValidator<Collection<?>, ValidationError> sizeBetween(int inclMin, int inclMax) {
        return Validators.sizeBetween(inclMin, inclMax, v -> error(COLLECTION_SIZE_NOT_BETWEEN_BOUNDS, inclMax, inclMax, v.size()));
     }

    public static KeyedValidator<String, ValidationError> lengthBetween(int inclMin, int inclMax) {
        return Validators.lengthBetween(inclMin, inclMax, v -> error(STRING_LENGTH_NOT_BETWEEN_BOUNDS, inclMin, inclMax, v.length()));
    }

    public static <T, E> KeyedValidator<T, ValidationError> required() {
        return Validators.required(() -> error(VALUE_REQUIRED));
    }

    public static <T, E> KeyedValidator<T, ValidationError> required(KeyedValidator<T, E> validator) {
        return Keyed.required(validator, () -> error(VALUE_REQUIRED));
    }

    public static <O, T> KeyedValidator<O, ValidationError> required(String key, 
                                                                     Function<O, T> fn, 
                                                                     KeyedValidator<T, ValidationError> validator) {
        return Keyed.required(key, fn, validator, () -> error(VALUE_REQUIRED));
    }

    public static <O, T, E> KeyedValidator<O, ValidationError> required(String key, 
                                                                        Function<O, T> fn) {
        return keyed(key, Keyed.required(fn, () -> error(VALUE_REQUIRED)));
    }
